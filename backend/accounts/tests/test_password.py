...
class PasswordRecoveryTests(TestCase):
    ...
    def test_reset_responses_do_not_enumerate_accounts(self, send_email):
        ...
        self.assertEqual(
            {call.kwargs["email"] for call in send_email.call_args_list},
            {self.user.email, oauth_user.email},
        )
    ...
    def test_reset_link_creates_first_password(self, send_email):
        """A completed passwordless account can set its first password."""
        passwordless_user = User.objects.create_user(
            username="passwordless-user",
            email="passwordless@example.com",
        )
        passwordless_user.set_unusable_password()
        passwordless_user.save(update_fields=["password"])
        Profile.objects.update_or_create(
        ...
        social_account = SocialAccount.objects.create(
            user=passwordless_user,
            provider="google",
            uid="passwordless-oauth-uid",
        )

        request_response = self.request_reset(passwordless_user.email)
        reset_link = send_email.call_args.kwargs
        ...
        self.assertTrue(passwordless_user.check_password(self.strong_password))
        self.assertTrue(
            SocialAccount.objects.filter(pk=social_account.pk).exists()
        )
        otp.store_code(passwordless_user.email, "482731")
        self.assertEqual(
            otp.verify_code(passwordless_user.email, "482731"),
            (True, ""),
        )

    def test_dedicated_password_setup_endpoints_are_removed(self):
        """The retired authenticated setup-code APIs are not callable."""
        anonymous = APIClient()

        send_response = anonymous.post(
        ...
        self.assertEqual(setup_response.status_code, status.HTTP_404_NOT_FOUND)
