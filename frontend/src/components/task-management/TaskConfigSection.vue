  ...
  try {
    ...
    if (data) {
      ...
      autoCleanupEnabled.value = !!(
        typeof data.cleanup_crontab === 'string' && data.cleanup_crontab.trim()
      )
      overtimeMarkingEnabled.value = !!(
        typeof data.mark_timeout_crontab === 'string' &&
        data.mark_timeout_crontab.trim()
      )
    }
  } catch (e) {
...
async function saveConfig() {
  ...
  try {
    ...
    if (data) {
      ...
      autoCleanupEnabled.value = !!(
        data.cleanup_crontab && String(data.cleanup_crontab).trim()
      )
      overtimeMarkingEnabled.value = !!(
