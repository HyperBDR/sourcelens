                ...
                <div class="divide-y divide-line rounded-xl border border-line">
                  <div class="flex items-start justify-between gap-4 px-4 py-4">
                    ...
                    <button
                      ...
                      class="relative mt-0.5 inline-flex h-6 w-11 shrink-0 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-200 focus:ring-offset-2"
                      :class="
                    ...
                    >
                      <span
                        class="pointer-events-none inline-block h-5 w-5 translate-y-0.5 rounded-full bg-white shadow-sm transition-transform"
                        :class="
                          preferencesStore.answerCompletionIndicator
                            ? 'translate-x-[22px]'
                            : 'translate-x-0.5'
                  ...
                  <div class="flex items-start justify-between gap-4 px-4 py-4">
                    <div class="min-w-0">
                      <div class="flex flex-wrap items-center gap-2">
                        <div class="text-sm font-medium text-gray-900">
                          {{ t('settings.modal.browserNotification') }}
                        </div>
                        <span
                          class="rounded-full px-2 py-0.5 text-[11px] font-medium"
                          :class="browserStateClass"
                        >
                          {{
                            t(
                              `settings.modal.notificationState.${browserState}`
                            )
                          }}
                        </span>
                      </div>
                      <p class="mt-1 text-xs leading-5 text-gray-500">
                        {{ t('settings.modal.browserNotificationDesc') }}
                      </p>
                    </div>
                    <button
                      type="button"
                      ...
                      :aria-label="t('settings.modal.browserNotification')"
                      :disabled="browserToggleDisabled"
                      class="relative mt-0.5 inline-flex h-6 w-11 shrink-0 rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-primary-200 focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50"
                      :class="
...
const notificationApi = computed(() => {
  permissionVersion.value
  return typeof window === 'undefined' ? null : window.Notification
})

const browserState = computed(() =>
  browserNotificationState({
    enabled: preferencesStore.browserCompletionNotification,
    isSecureContext:
      typeof window !== 'undefined' && window.isSecureContext === true,
    notificationApi: notificationApi.value
  })
)
...
const browserToggleDisabled = computed(
  () =>
    requestingPermission.value ||
    ['blocked', 'unsupported'].includes(browserState.value)
)

const browserStateClass = computed(() => {
  if (browserState.value === 'enabled') {
    return 'bg-emerald-50 text-emerald-700'
  }
  if (browserState.value === 'blocked') {
    return 'bg-amber-50 text-amber-700'
  }
  if (browserState.value === 'unsupported') {
...
const toggleCompletionIndicator = () => {
...
const toggleBrowserNotification = async () => {
  if (browserNotificationEnabled.value) {
    preferencesStore.setBrowserCompletionNotification(false)
    return
  }
  ...
  if (state === 'enabled') {
    preferencesStore.setBrowserCompletionNotification(true)
  }
...
watch(
  ...
  (show) => {
    ...
    if (show) permissionVersion.value += 1
  },
  { immediate: true }
)
